...
const getDBRefreshTokenByRefreshToken = `-- name: GetDBRefreshTokenByRefreshToken :one
SELECT token, user_id, created_at, updated_at, expires_at, revoked_at FROM refresh_tokens WHERE token=$1
`

func (q *Queries) GetDBRefreshTokenByRefreshToken(ctx context.Context, token string) (RefreshToken, error) {
	row := q.db.QueryRowContext(ctx, getDBRefreshTokenByRefreshToken, token)
	var i RefreshToken
	err := row.Scan(
		&i.Token,
		&i.UserID,
